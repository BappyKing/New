...
girl = ['jyoti Sharam', 'kajal kashyap', 'priya Rajput', 'salini Kashyap', 'Rosanj Saini', 'Sapna kashap', 'Sajida Malik', 'Ayesha Khan', 'Nabeela Malik', 'Kinza Fatima', 'Arooj Khan', 'Muskan Khan', 'Ayesha Malik', 'Safina Malik', 'Nida Ali', 'Rimsha Ali']
ok = []
cp = []
def menu():
    os.system('clear')
...
class create:
    ...
    'viewport-width': '980',}
        headers1 =  {'authority': 'mbasic.facebook.com',
        'method':'POST',
        'scheme':'https',
    'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
    ...
    'viewport-width': '980',}
        ...
        for x in range(lim):
            self.loop += 1
            sys.stdout.write(f'\r{OO}[HONEY-XD] {OO}{self.loop}/{str(lim)} OK: {len(ok)} = CP: {len(cp)}{OO} '),
            sys.stdout.flush()
                        ...
                        for x in BeautifulSoup(po.text, 'html.parser').find_all('form', {'method': 'post'}):
                            if 'confirmation_event_location=cliff' in str(x):
                                ...
                                if 'checkpoint' in rex.url:
                                    cok = self.ses.cookies.get_dict()
                                    cp.append(email+passw)
                                    print ('\r\033[1;33m[HONEY-CP]033[1;34m '+cok['c_user']+' | 033[1;35m'+passw+'\033[0;97m     ')
                                else:
                                    coki = (";").join([ "%s=%s" % (key, value) for key, value in self.ses.cookies.get_dict().items() ])
                                    cok = self.ses.cookies.get_dict()
                                    print ('\r\033[1;32m[HONEY-OK]\033[1;33m '+cok['c_user']+' |\033[1;34m'+passw+' | \033[1;33m'+coki+'\033[0;97m     ')
                                    open('/sdcard/HONEY-COOKIE.txt','a').write(ids+'|'+pas+ ' | ' +coki+'\n')
                                    open('/sdcard/HONEY-OK.txt','a').write(ids+'|'+pas+'|'+coki+'\n')
